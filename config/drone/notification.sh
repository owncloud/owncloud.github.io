...
if [[ "$DRONE_BUILD_STATUS" == "success" ]]; then
    log_success "Build successful skipping notification step"
    exit 0
fi

message_html='<b>❌️ Failure</b> <a href="'${DRONE_BUILD_LINK}'">'${DRONE_REPO}'#'$COMMIT_SHA_SHORT'</a> ('${BUILD_SOURCE}') by <b>'${DRONE_COMMIT_AUTHOR}'</b>'
